...
class Graph(Widget):
    ...
    _mesh_ticks = ObjectProperty()
    # the mesh which draws the surrounding rectangle
    _mesh_rect = ObjectProperty()
    ...
    @staticmethod
    def _get_ticks(major, minor, log, s_min, s_max):
        if major and s_max > s_min:
            if log:
                ...
                if floor(s_min + n_decades) != floor(s_max):
                    n_decades += 1 - (10 ** (s_min + n_decades + 1) - 10 ** s_max) / 10 ** floor(s_max + 1)
                else:
                    n_decades += ((10 ** s_max - 10 ** (s_min + n_decades)) / 10 ** floor(s_max + 1))
                ...
                start_dec = ceil((10 ** Decimal(s_min - s_min_low - 1)) / Decimal(decade_dist)) * decade_dist
    ...
    def _update_labels(self):
        ...
        if len(ylabels) and ylabel_grid:
            ...
            y_start = y_next + (padding + y1[1] if len(xlabels) and xlabel_grid
                                else 0) + (padding + y1[1] if not y_next else 0)
            ...
            for k in range(len(ylabels)):
                ...
                ylabels[k].pos = (int(x_next), int(y_start + (ypoints[k] - ymin) * ratio))
        ...
        if len(xlabels) and xlabel_grid:
            ...
            if not x_next:
                xlabels[0].text = precision % funcexp(xpoints[0]) + " " + self.x_unit
                xlabels[0].texture_update()
            ...
            for k in range(len(xlabels)):
                xlabels[k].text = precision % funcexp(xpoints[k]) + " " + self.x_unit
                ...
                xlabels[k].pos = (int(x_next + (xpoints[k] - xmin) * ratio - half_ts), int(y_next))
    ...
    def _redraw_all(self, *args):
        ...
        mesh = self._mesh_ticks
        n_points = (len(xpoints_major) + len(xpoints_minor) + len(ypoints_major) + len(ypoints_minor))
    ...
    def _redraw_x(self, *args):
        ...
        for k in range(grid_len, n_labels):
            grids[k] = GraphRotatedLabel(font_size=font_size, angle=self.x_ticks_angle, **self.label_options)
    ...
    def collide_plot(self, x, y):
        ...
        return 0 <= adj_x <= self._plot_area.size[0] and 0 <= adj_y <= self._plot_area.size[1]
...
class LinePlot(Plot):
    ...
    def create_drawings(self):
        ...
        with self._grc:
            ...
            self._gline = Line(points=[], cap='none', width=self.line_width, joint='round')
        return [self._grc]
